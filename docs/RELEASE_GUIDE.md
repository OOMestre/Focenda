...
- Packages application into native drag-and-drop installer `Focenda-macOS.dmg`.
- Detects whether the tag is a beta (`is_prerelease=true`) or production release (`is_prerelease=false`).
- Publishes the GitHub Release with the attached `Focenda-macOS.dmg` binary and generated release notes.
